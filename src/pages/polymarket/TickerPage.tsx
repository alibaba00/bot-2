...
import { fetchMarket, fetchMarkets, fetchMarketBySlugFromGamma, fetchMarketPricesFromClob } from '@/lib/polymarket/markets'
import type { Market } from '@/lib/polymarket/types'
import { Activity, TrendingUp, TrendingDown, Wifi, WifiOff, AlertCircle, Play, Square } from 'lucide-react'
...
const DEFAULT_BTC_MARKET_SLUGS = [
	'btc-updown-15m-1764256500',
]
...
export default function TickerPage() {
	...
	const [priceData, setPriceData] = useState<Record<string, { price: number; timestamp: number }>>({})
		...
		autoConnect: false, // We'll connect manually
			...
			useEffect(() => {
					...
					try {
						...
						for (const slug of DEFAULT_BTC_MARKET_SLUGS) {
							try {
								...
								if (fetchedMarket && fetchedMarket.id && fetchedMarket.question && fetchedMarket.question.trim() !== '') {
								...
								console.log(`Gamma API by slug failed for ${slug}:`, slugError?.message || slugError)
							}
						}
						...
						if (!marketData) {
							...
							for (const conditionId of DEFAULT_BTC_MARKET_CONDITION_IDS) {
								try {
									...
									if (fetchedMarket && fetchedMarket.id && fetchedMarket.question && fetchedMarket.question.trim() !== '') {
										marketData = fetchedMarket
										console.log(`✅ Found market via direct API with condition ID: ${conditionId}`, {
											question: marketData.question,
											id: marketData.id,
											conditionId: marketData.conditionId,
											outcomesCount: marketData.outcomes?.length || 0
										...
										if (fetchedMarket) {
											console.log(`⚠️ Market object incomplete for ID ${conditionId}:`, {
									...
									const errorMsg = idError?.message || idError?.data?.error || String(idError)
									console.log(`❌ Direct API call failed for ID ${conditionId}:`, errorMsg)
						...
						if (!marketData) {
							...
							try {
								...
								if (!marketData) {
								console.log('Market not found by ID, searching for Bitcoin 15min markets...')
								...
								let btcMarkets = allMarkets.filter(m => {
									...
									return (
										...
										(question.includes('15m') || question.includes('15min') || question.includes('15 min') || question.includes('up or down'))
									)
								})
								
								console.log(`Found ${btcMarkets.length} Bitcoin 15min markets (before filtering active/closed)`)
								
								// Log some examples
								if (btcMarkets.length > 0) {
									console.log('Sample Bitcoin 15min markets:', btcMarkets.slice(0, 3).map(m => ({
										question: m.question,
										id: m.id,
								...
								if (activeBtcMarkets.length > 0) {
									...
									if (anyBtcMarket) {
										...
										console.log('⚠️ Found Bitcoin market (fallback, may not be 15min):', {
											question: marketData.question,
											id: marketData.id,
											conditionId: marketData.conditionId,
											active: marketData.active,
				...
				if (marketData) {
					...
					setError('Bitcoin 15min market not found. Please check if the market is active.')
				}
			} catch (error) {
	...
	useEffect(() => {
		...
		const pollPrices = async () => {
			try {
				pollCount++
				console.log(`🔄 Polling prices (attempt ${pollCount}, using ${useClobApi ? 'CLOB' : 'Gamma'} API)...`)
						...
						if (clobPrices && Object.keys(clobPrices).length > 0) {
							...
							market.outcomes.forEach((outcome) => {
								...
								const price = clobPrices[outcome.id] || 
									Object.values(clobPrices)[0] // Fallback to first price if no match
							...
							if (Object.keys(newPriceData).length > 0) {
								setPriceData((prevPriceData) => {
									...
									Object.keys(newPriceData).forEach((outcomeId) => {
										...
										if (oldPrice !== undefined && Math.abs(oldPrice - newPrice) > 0.0001) {
											hasChanges = true
											const outcome = market.outcomes.find(o => o.id === outcomeId)
											console.log(`📊 Price change detected for ${outcome?.title || outcomeId}: ${(oldPrice * 100).toFixed(2)}% → ${(newPrice * 100).toFixed(2)}%`)
										}
									})
									
									if (hasChanges || pollCount === 1) {
										console.log('✅ Prices updated via CLOB API polling', {
											prices: Object.entries(newPriceData).map(([id, data]) => {
												const outcome = market.outcomes.find(o => o.id === id)
						...
						setPriceData((prevPriceData) => {
							...
							updatedMarket.outcomes.forEach((outcome) => {
								if (outcome.id) {
									...
									if (oldPrice !== undefined && Math.abs(oldPrice - newPrice) > 0.0001) {
										hasChanges = true
										console.log(`📊 Price change detected for ${outcome.title}: ${(oldPrice * 100).toFixed(2)}% → ${(newPrice * 100).toFixed(2)}%`)
									}
							...
							if (hasChanges || pollCount === 1) {
								console.log('✅ Prices updated via Gamma API polling', {
									prices: updatedMarket.outcomes.map(o => `${o.title}: ${(o.price * 100).toFixed(2)}%`)
								})
					...
					<CardContent>
						<div className="space-y-2">
							...
							<p className="text-sm text-muted-foreground">
								The market may be closed, inactive, or the market ID may have changed.
							</p>
						</div>
					</CardContent>
				</Card>
					...
					{assetIds.length > 0 && (
						<div className="flex items-center gap-2 flex-wrap">
							...
							<div className="flex items-center gap-2 border-r pr-2">
								<div className={cn('flex items-center gap-2', getStatusColor(ws1.status))}>
							...
							<div className="flex items-center gap-2 border-r pr-2">
								<div className={cn('flex items-center gap-2', getStatusColor(ws2.status))}>
											...
											<div>
												...
												{priceData[outcome.id] && (
													<div className="text-xs text-muted-foreground mt-1">
														Updated: {new Date(priceData[outcome.id].timestamp).toLocaleTimeString()}
													</div>
												)}
											</div>
											<div className={cn(
												'p-3 rounded-full',
												isUp ? 'bg-green-100 dark:bg-green-900/20' : 'bg-red-100 dark:bg-red-900/20'
											)}>
												{isUp ? (
						...
						<div className="mt-4 pt-4 border-t space-y-2">
							{wsError1 && (
								<div className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
									...
									<p className="text-sm text-red-700 dark:text-red-300 mt-1">{wsError1}</p>
								</div>
							)}
							{wsError2 && (
								<div className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
									...
									<p className="text-sm text-red-700 dark:text-red-300 mt-1">{wsError2}</p>
								</div>
							)}
						</div>
					)}
						...
						<div className="mt-4 pt-4 border-t">
							<div className="text-sm text-muted-foreground space-y-1">
								{ws1.lastPriceUpdate && (
									<div className="flex items-center gap-2">
										<Activity className="h-4 w-4" />
										<span>WS1 last update: {new Date(ws1.lastPriceUpdate.timestamp).toLocaleTimeString()}</span>
									</div>
								)}
								{ws2.lastPriceUpdate && (
									<div className="flex items-center gap-2">
										<Activity className="h-4 w-4" />
										<span>WS2 last update: {new Date(ws2.lastPriceUpdate.timestamp).toLocaleTimeString()}</span>
									</div>
								)}
							</div>
						</div>
					...
					{assetIds.length > 0 && (
						<div className="mt-2 pt-2 border-t">
							<div className="text-xs text-muted-foreground">
								Asset IDs: {assetIds.length} token{assetIds.length !== 1 ? 's' : ''} loaded
							</div>
						</div>
					)}
					</CardContent>
