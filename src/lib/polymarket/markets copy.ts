...
export async function getTickSize(conditionId: string): Promise<number | null> {
	try {
		const client = await getOrInitializeClient()
		const marketData = await client.getMarket(conditionId)
		return marketData.tickSize || 0.01
	} catch (error) {
		console.error('Error getting tick size:', error)
		return null
	}
}

export async function getNegRisk(conditionId: string): Promise<boolean | null> {
	try {
		const client = await getOrInitializeClient()
		const marketData = await client.getMarket(conditionId)
		return marketData.negRisk || false
	} catch (error) {
		console.error('Error getting neg risk:', error)
		return null
	}
}


export async function getMarket(conditionId: string): Promise<MarketData | null> {
	try {
		const client = await getOrInitializeClient()
		const marketData = await client.getMarket(conditionId)
		return marketData
...
export async function setOrder(conditionId: string, price: number, size: number, side: Side): Promise<string | null> {
	const market = await getOrInitializeClient()
	.then(async (client: any) => await client.getMarket(conditionId))
	.then((market: any) => market);

	const response = await getOrInitializeClient()
	.then(async (client: any) => await client.createAndPostOrder(
		{
			tokenID: conditionId,
			price: price,
			size: size,
	...
	.then((response: any) => response.orderID);
